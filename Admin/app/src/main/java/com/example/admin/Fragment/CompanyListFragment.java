...
public class CompanyListFragment extends Fragment {
    ...
    @BindView(R.id.companylist_recyclerview)
    ...
    private List<Company_Model> list = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        firebaseDatabase.child("Company").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists())
                {
                    for (DataSnapshot postSnapshot : snapshot.getChildren()) {
                        ...
                        Company_Model model = postSnapshot.getValue(Company_Model.class);
                        list.add(model);
                    }

                    adapter = new Company_Adapter(getActivity(),list);
